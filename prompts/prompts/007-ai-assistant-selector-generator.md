# Prompt: AI Assistant Web Page Selector Generator

This prompt guides an AI agent in creating a JSON file of CSS selectors for a specified AI assistant website. The generated JSON will be used to extract conversation data.
...
### Version 1: For Browser Agents (with direct DOM access)
...
### Version 2: For Playwright Agents

**Workflow:**

1.  **Navigate to URL:** The user will provide a starting URL. Use Playwright to navigate to this page.
2.  **Identify Key Elements:** Locate the HTML elements corresponding to the fields in the `AiAssistantSiteSpec` schema (`loginIndicator`, `userInputBox`, etc.) for both the `mainPage` and the `chatPage`. You may need to navigate between pages.
3LAGI.  **Craft Selectors:** For each element, write a robust CSS selector.
    *   **Prioritize stability:** Prefer selectors using `data-testid`, `id`, or other unique and stable attributes over fragile, auto-generated class names.
    *   **Use fallbacks:** If possible, provide an array of selectors as fallbacks.
    *   **Test your selectors:** Use Playwright's locator functions (e.g., `page.locator('...')`) and assertions (`expect(...).toBeVisible()`) to verify that each selector identifies the correct element.
4.  **Handle Missing Elements:** If you cannot find a selector for a specific element, omit it from the JSON output and report it to the user.
5.  **Address Dynamic Content & Logins:**
    *   Be aware that some elements might only appear after a user action (e.g., login).
    *   If you encounter a login wall or suspect more content is available after login, pause your work, inform the user, and await further instructions or for them to perform the login.
6.  **Construct the JSON:** Assemble the selectors into a single JSON object matching the `AiAssistantSiteSpec` schema.
7.  **Present and Iterate:**
    *   Show the user the complete JSON output.
    *   Report any selectors you were unable to find.
...
Please fill out the following template to request a new selector set.

```
# User Request

**AI Assistant Website URL:** [e.g., https://gemini.google.com/app]

**Additional Information:** [e.g., "The site requires login, which I will handle.", "Please start with the chat page selectors.", "The `messageBlocks` are the most important part."]
```